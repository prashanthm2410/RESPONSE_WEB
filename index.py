...
app = Flask(__name__, static_folder='prod_price_pred/static', template_folder='prod_price_pred/templates')

# Load models and vectorizers
model_path = os.path.join('prod_price_pred', 'models', 'xgboost_model.pkl')
vectorizer_path = os.path.join('prod_price_pred', 'models', 'tfidf_vectorizer_product_rating_xgb.pkl')
...
def submit():
    ...
    chosen_category_redirect = " ".join([option_content_mapping1.get(str(int(category1)), 'Unknown'),
                                         option_content_mapping2.get(str(int(category2)), 'Unknown'),
                                         option_content_mapping3.get(str(int(category3)), 'Unknown')])
...
if __name__ == '__main__':
    port = int(os.environ.get('PORT', 5000))  # Use the port specified by the environment or default to 5000
    app.run(host='0.0.0.0', port=port)  # Bind to all IP addresses
